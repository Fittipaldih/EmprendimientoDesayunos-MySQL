...
DELETE FROM Receta WHERE IdComponente = 4;
-- Si no sabriamos el ID :
DELETE FROM Receta WHERE EXISTS(SELECT 1 
				FROM Componente
...
-- Si sabemos la PK seria:
UPDATE Tema_Producido 
SET IdTema = 4 -- 'Alicornio'
WHERE IdTema = 3; -- 'Unicornio'

-- Si es necesario realizar la busqueda por el texto seria:
UPDATE Tema_Producido 
SET IdTema = (	SELECT IdTema
	        FROM Tema 
    		WHERE Nombre like 'Alicornio')
WHERE IdTema = (SELECT IdTema 
		FROM Tema 
    		WHERE Nombre like 'Unicornio');

/*** DML ***/
-- 4. Obtener todas las recetas por componente, paso e ingrediente.
SELECT C.Nombre, R.Paso, I.Nombre,  R.Cantidad, R.Medida, R.Elaboracion
FROM Receta R
